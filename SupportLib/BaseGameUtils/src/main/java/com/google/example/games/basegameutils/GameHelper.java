...
public class GameHelper implements GoogleApiClient.ConnectionCallbacks,
    ...
    public static GameHelper mInstance;
    ...
    public final static String PARAM_PAYLOAD = "PAY";
    public final static String PARAM_DAY = "DAY";
    public final static String PARAM_BITMAP = "BITM";
    public final static String PARAM_REQUEST_TYPE = "RTYPE";
    public final static String PARAM_REQUEST_POSITION = "RPOSITION";
    ...
    // GIFT INBOX
    GameRequestBuffer bufGift;
    GameRequestBuffer bufWish;
    ...
    public GameHelper(Activity activity, int clientsToUse) {
        mActivity = activity;
        Log.d(TAG,mActivity.getClass().getSimpleName());
    ...
    public void getGiftInbox() {
        Intent target = new Intent(mAppContext, GiftResultActivity.class);
        target.putExtra(PARAM_TYPE, GiftResultActivity.SHOW_INBOX);
        mActivity.startActivity(target);
    }

    public void sendGift(String description, String bitmap, String payload, int time) {
        Intent target = new Intent(mAppContext, GiftResultActivity.class);
        target.putExtra(PARAM_TYPE,  GiftResultActivity.SEND_GIFT_CODE);
        target.putExtra(PARAM_DESCRIPTION, description);
        target.putExtra(PARAM_BITMAP, bitmap);
        target.putExtra(PARAM_PAYLOAD, payload);
        target.putExtra(PARAM_DAY, time);
        mActivity.startActivity(target);
    }

    public void wishGift(String description, String bitmap, String payload, int time) {
        Intent target = new Intent(mAppContext, GiftResultActivity.class);
        target.putExtra(PARAM_TYPE, GiftResultActivity.WISH_GIFT_CODE);
        target.putExtra(PARAM_DESCRIPTION, description);
        target.putExtra(PARAM_BITMAP, bitmap);
        target.putExtra(PARAM_PAYLOAD, payload);
        target.putExtra(PARAM_DAY, time);
    ...
    public String getToken() {
        ...
        debugLog("Should get TOKEN");
        if ((mRequestedClients & CLIENT_GAMES) != 0) {
            String accountName = Games.getCurrentAccountName(mGoogleApiClient);
            String scope =  "oauth2:"+ "https://www.googleapis.com/auth/games"; //Games.SCOPE_GAMES.toString();

            debugLog("Find TOKEN  for : " +  accountName);

            try {
                String token = GoogleAuthUtil.getToken(mActivity,accountName,scope);

                debugLog("GENERATE TOKEN : " +  token);
        ...
        else if ((mRequestedClients & CLIENT_PLUS) != 0)
        {
            debugLog("Should try CLIENT_PLUS");
        }
        return null;
    }
}
